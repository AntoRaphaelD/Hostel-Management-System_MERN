...
const { 
  ...
  Hostel, IncomeType,Store, ItemStore, InventoryTransaction
} = require('../models');
...
const scheduleMenu = async (req, res) => {
  try {
    ...
    if (!menu_id || !scheduled_date || !meal_time) {
      return res.status(400).json({
        success: false,
        message: 'Menu ID, scheduled date, and meal time are required'
      });
    }

    // Check if menu exists and belongs to hostel
    const menu = await Menu.findOne({
      where: { id: menu_id, hostel_id }
    });
    ...
    res.status(201).json({
      success: true,
      data: schedule,
      message: 'Menu scheduled successfully'
    });
  } catch (error) {
...
const getMenuSchedule = async (req, res) => {
  try {
    const { date, meal_time } = req.query;
    const hostel_id = req.user.hostel_id;

    let whereClause = { hostel_id };

    if (date) whereClause.scheduled_date = date;
    if (meal_time) whereClause.meal_time = meal_time;
...
// BULK CONSUMPTION MANAGEMENT
// In your recordBulkConsumption function
const recordBulkConsumption = async (req, res) => {
  try {
    ...
    try {
      // Create all consumption records and update stocks
      ...
      for (const consumption of consumptions) {
        ...
        // Create consumption record - CHANGE HERE from 'daily' to 'dinner'
        const record = await DailyConsumption.create({
          hostel_id,
          item_id,
          consumption_date: consumption_date || new Date(),
          quantity_consumed,
          unit,
          meal_type: 'dinner', // Change from 'daily' to one of the allowed values
          recorded_by: req.user.id
        }, { transaction });

        // Rest of your code remains the same
        createdConsumptions.push(record);
        ...
        if (itemStock) {
          ...
          itemStock = await ItemStock.create({
            item_id,
            hostel_id,
            current_stock: 0, // Since we've consumed it all
            minimum_stock: 0,
            last_updated: new Date()
          }, { transaction });
  ...
  try {
    ...
    try {
      ...
      for (const item of items) {
        ...
        createdTransactions.push(inventoryTx);
        ...
        if (itemStock) {
          const newStock = parseFloat(itemStock.current_stock) + parseFloat(quantity);
          await itemStock.update({
            current_stock: newStock,
            last_updated: new Date()
          }, { transaction });
        ...
        // Update the last purchased date in ItemStore mapping
        if (store_id) {
          const itemStore = await ItemStore.findOne({
            where: { item_id, store_id },
            transaction
          });
          ...
          if (itemStore) {
            await itemStore.update({
              ...
              price: unit_price || itemStore.price
            }, { transaction });
          } else {
      ...
      res.status(201).json({
        ...
        message: 'Inventory purchases recorded successfully'
      });
    } catch (error) {
      await transaction.rollback();
      throw error;
